...
class BaseClient {
    ...
    /**
     * Request a file from a device (by socket)
     * @param { string } device 
     * @param { string } path 
     * @returns { NodResponse }
     */
    requestFile = async (device, path) => {
        this.send(new Message(this.deviceName, device, 'files/request', { path }));
        return {
     ...
     * @param { string } path 
     */
    sendFile = async (to, path) => {
        
    }

    /**
     * Get shared folder based on network path
     * @param { SharedFolder[] } folders
     * @param { string } path 
     */
    static getSharedFolderByNetworkPath = (folders, path) => {
        const normalizedPath = path[0] === '/' ? path.substring(1) : path;
        const folder = folders.find((folder) => {
            return normalizedPath.startsWith(folder.name);
        })
        return folder;
    }

    /**
     * Derive a local path to a shared file by network path
     * @param { string  } sharedFolderFullPath path of a shared folder
     * @param { string } networkPath 
     */
    static deriveSharedFileLocalPathByNetworkPath = (sharedFolderFullPath, networkPath) => {
        const networkPathArr = networkPath.split('/');
        networkPathArr.shift();
        const networkPathTrimmed = networkPathArr.join('/');
        return sharedFolderFullPath + '/' + networkPathTrimmed;
    }
    ...
    onMessage = (message) => {
        ...
        try {
            // Todo: `getMessageHandler(type)` or `handleMessage(message)`
            ...
            } else if (message.type === 'files/request') {
                const folder = this.getSharedFolderByNetworkPath(this.sharedFolders, message.data.path);
                const localPathToFile = this.deriveSharedFileLocalPathByNetworkPath(folder.fullPath, message.data.path);
                const file = fs
            }
        } catch (e) {
